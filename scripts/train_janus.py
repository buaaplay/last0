...
class SftDataset(Dataset):
    ...
    def collate_fn(self, batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        for x in batch:
            ...
            user_content = input_img_tokens_1 * img_len + x['input_prompt'] + state_tokens
            gpt_content = action_tokens + input_img_tokens_2
            conversation = [
...
def train(args: argparse.Namespace) -> None:
    ...
    for epoch in range(0, args.n_epochs):
        ...
        for batch in train_iter:
            ...
            action_tokens = batch['input_ids'][:, -(image_embeds.shape[1]*2 + 4 + args.action_dim) : -(image_embeds.shape[1]*2 + 4)]
            ...
            action_logits = model.language_model.lm_head(hidden_states[:, -(image_embeds.shape[1]*2 + 5 + args.action_dim) : -(image_embeds.shape[1]*2 + 5), :])
