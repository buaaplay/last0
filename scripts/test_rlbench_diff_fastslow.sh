...
models=("/media/liuzhuoyang/LCoT_VLA_MOT/exp/latent_cot_mot_diff/janus_pro_siglip_uni3d_1B_1e-4_mot_slow1_fast4_chunk4_pc_state_20_wonoise_1215/stage2/checkpoint-299-33900/tfmr")
...
for model in "${models[@]}"; do
  ...
  for task in "${tasks[@]}"; do
    python /media/liuzhuoyang/LCoT_VLA_MOT/scripts/test_rlbench_siglip_diff_fastslow.py \
      --model-path ${model} \
      --task-name ${task} \
      --exp-name ${exp_name} \
      --result-dir ${model} \
      ...
      --fs_ratio 4 \
      --result-dir /media/liuzhuoyang/LCoT_VLA_MOT/test_results/test_1217_mot \
      --action-chunk 1
  done
done
