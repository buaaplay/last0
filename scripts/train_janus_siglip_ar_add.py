...
def train(args: argparse.Namespace) -> None:
    ...
    for epoch in range(0, args.n_epochs):
        ...
        for batch in train_iter:
            ...
            if args.use_latent:
                bs, n = batch['latent_pixel_values'].shape[0:2]  # B, n_images
                # [B, n_images, 3, H, W] → [B*n, 3, H, W]
                ...
                # [B*n, T_img, D] → [B, n_images, T_img, D]
                latent_embeds = rearrange(latent_embeds, "(b n) t d -> b n t d", b=bs, n=n)
                B, N, T_img, D = latent_embeds.shape  # token_per_image = T_img
                latent_size = args.latent_size
                compress_strategy = args.compress_strategy
                # final GT: [B, latent_size, D]
                final_gt = []
                for b in range(B):
                    per_img_compressed = []  
                    for i in range(N):
                        feats = latent_embeds[b, i]  # [T_img, D]
                        if T_img != latent_size:
                            if compress_strategy == "average":
                                group_size = max(1, T_img // latent_size)
                                res = T_img % latent_size
                                if res > 0:
                                    feats = feats[:-res, :]
                        ...
                        per_img_compressed.append(feats)  # each is [latent_size, D]
                    summed = torch.stack(per_img_compressed, dim=0).sum(dim=0)
                    final_gt.append(summed)
                compressed_latent_embeds = torch.stack(final_gt, dim=0).to(inputs_embeds.dtype)

                # print("compressed_latent_embeds.shape", compressed_latent_embeds.shape)
                # input("check compressed latent embeds")
