    ...
    def collate_fn(self, batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        if self.config.use_latent: # only for multi image
            ...
            for x in batch:
                ...
                # output_state 列表，通常长度为 4
                for single_state in x['output_state']:
                    # A. 归一化 (复制 input state 的逻辑)
        ...
        return {
            ...
            "latent_pixel_values": latent_pixel_values, # # [B, chunk_size, 3, H, W]
    ...
    for epoch in range(0, args.n_epochs):
        ...
        for batch in train_iter:
            ...
            if args.use_latent:
                ...
                bs, n_future = batch['latent_pixel_values'].shape[0:2]  # [B, 4, ...]
                n_imgs = batch['latent_pixel_values'].shape[1]
                ...
                img_embeds = rearrange(img_embeds_flat, "(b n) t d -> b n t d", b=bs, n=n_future) # [B, 4, 576, D]
                # Compress image embedding (Average Pooling)
                target_img_tokens = (args.latent_size // n_imgs) - 1 # -1 for state token
                T_vis = img_embeds.shape[2]
                group_size = T_vis // target_img_tokens
                img_chunks = torch.split(img_embeds, group_size, dim=2)
                compressed_imgs = torch.cat([c.mean(dim=2, keepdim=True) for c in img_chunks[:target_img_tokens]], dim=2) # # [B, 4, 4, D]
                ...
                # Merge compressed image and state embeddings
                combined_embeds = torch.cat([compressed_imgs, compressed_state], dim=2) # [B, 4, 5, D]
                compressed_latent_embeds = rearrange(combined_embeds, "b n k d -> b (n k) d") # Flatten: [B, 20, D]
                compressed_latent_embeds = compressed_latent_embeds.to(inputs_embeds.dtype)
                # print("compressed_latent_embeds.shape", compressed_latent_embeds.shape)
                # input("wait")
