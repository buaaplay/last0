...
class SftDataset(Dataset):
    ...
    def collate_fn(self, batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        for x in batch:
            ...
            pc_len = len(x['input_pointcloud']) if 'input_pointcloud' in x else 0 
            ...
            img_part = input_img_tokens * img_len
            latent_str = latent_start_str + latent_pad_str + latent_end_str if self.config.use_latent else ""
            
            prompts = img_part + x['input_prompt'] + state_tokens + latent_str + action_tokens
...
def train(args: argparse.Namespace) -> None:
    ...
    # raw_model.pointcloud_embedder = raw_model.pointcloud_embedder.float() # float32 for pointcloud embedder
    ...
    for epoch in range(0, args.n_epochs):
        ...
        for batch in train_iter:
            ...
            if args.use_latent:
                ...
                tokens_per_modality = args.latent_size // (num_frames * 2)
                ...
                combined_embeds = torch.cat([compressed_imgs, compressed_pcs], dim=2)
