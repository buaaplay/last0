...
def model_load(args):
    ...
    statistics_path = "/media/chenhao/double_rl/training_data/json/4tasks_train_statistics.json"
...
def model_predict(args, vl_gpt, vl_chat_processor, action_tokenizer, statistic, task_description, image, state, pointcloud, pre_image_dir, step):
    ...
    with torch.inference_mode():
        ...
        add_tokens = torch.cat([generate_ids, torch.tensor([[100001, 100016]]*generate_ids.shape[0]).to(device)], dim=-1)
    ...
    for i in range(args.num_episodes):
        ...
        for j in range(episode_length):
            ...
            if args.replay_or_predict == 'replay':
                ...
                Image.fromarray(obs_dict['image']).save(f"/media/chenhao/step{j}.png")
