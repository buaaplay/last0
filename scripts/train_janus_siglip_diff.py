...
class SftDataset(Dataset):
    ...
    def collate_fn(self, batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        timesteps = torch.randint(
            0, 
            self.model.diffusion.num_timesteps, 
            (normalized_actions.shape[0],)
        )
        noisy_actions = self.model.diffusion.q_sample(normalized_actions, timesteps, noise).to(torch.bfloat16)
        ...
        for x in batch:
            ...
            prompts = input_img_tokens * img_len + x['input_prompt'] + state_tokens + latent_str
...
def train(args: argparse.Namespace) -> None:
    ...
    if args.use_latent and not args.freeze_latent: # pointcloud embedder initialization
    ...
    for name, param in model.named_parameters():
        if '_action' in name:
            ...
            if args.use_latent and args.freeze_latent:
                if 'lm_head' in name or 'embed_tokens' in name or 'x_embedder' in name \
                 or 't_embedder' in name or 'final_layer' in name: # important!: action also need lm_head to decode
                    param.requires_grad = True
                else:
                    param.requires_grad = False
            else:
    ...
    for epoch in range(0, args.n_epochs):
        ...
        for batch in train_iter:
            ...
            latent_indexes, action_indexes = create_component_indexes(inputs_embeds.shape[1], 3) # important: 3 means <latent_end>, <timestep>, <noise>
