...
models=("/media/liuzhuoyang/LCoT_VLA_MOT/exp/latent_cot_mot/janus_pro_siglip_uni3d_1B_1e-4_mot_chunk4_pc1_1211/stage2/checkpoint-39-1160/tfmr")
...
for model in "${models[@]}"; do
  ...
  for task in "${tasks[@]}"; do
    python /media/liuzhuoyang/LCoT_VLA_MOT/scripts/test_rlbench_siglip_ar.py \
      --model-path ${model} \
      --task-name ${task} \
      --exp-name ${exp_name} \
      --result-dir ${model} \
      ...
      --result-dir /media/liuzhuoyang/LCoT_VLA_MOT/test_results/test_1211_mot \
      --action-chunk 1
  done
done
