...
models=("/media/liuzhuoyang/LCoT_VLA_MOT/exp/latent_cot_mot_flow/janus_pro_siglip_uni3d_1B_1e-4_mot_latent_pc_state_12_1221/stage2/checkpoint-249-7250/tfmr")
...
for model in "${models[@]}"; do
  ...
  for task in "${tasks[@]}"; do
    python /media/liuzhuoyang/LCoT_VLA_MOT/scripts/test_rlbench_siglip_flow.py \
      --model-path ${model} \
      --task-name ${task} \
      --exp-name ${exp_name} \
      --result-dir ${model} \
      ...
      --result-dir /media/liuzhuoyang/LCoT_VLA_MOT/test_results/test_1222_mot \
      --action-chunk 1
  done
done
