...
def train(args: argparse.Namespace) -> None:
    ...
    action_intermediate_size = args.action_intermediate_size if args.action_intermediate_size > 0 else None
    accelerator.print(f"Action intermediate size: {action_intermediate_size}")
    ...
    action_expert_params = sum(p.numel() for n, p in model.named_parameters() if '_action' in n)
    latent_expert_params = sum(
        p.numel() for n, p in model.named_parameters()
        if any(k in n for k in ['self_attn.q_proj.', 'self_attn.k_proj.', 'self_attn.v_proj.', 'self_attn.o_proj.'])
        and '_action' not in n
        and 'language_model.model.layers' in n
    ) + sum(
        p.numel() for n, p in model.named_parameters()
        if 'mlp.' in n and '_action' not in n and 'mlp_action' not in n
        and 'language_model.model.layers' in n
    )
    ...
    accelerator.print(f"Action intermediate size: {action_intermediate_size}")
    accelerator.print(f"Action expert parameters: {action_expert_params/1e6:.2f}M")
    accelerator.print(f"Latent expert parameters (attn+mlp): {latent_expert_params/1e6:.2f}M")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--action_intermediate_size', type=int, default=0,
                        help='Intermediate size for action expert MLP. 0 = use same as latent expert (default). '
                             'E.g. 2816 for half, 2048 for hidden_size, 1408 for quarter.')
