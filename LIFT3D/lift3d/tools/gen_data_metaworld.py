...
def main(args):
    ...
    while episode_idx < args.num_episodes:
        ...
        while not truncated:
            ...
            # obs_img.save(f'/workspaces/chenhao/code/lift3d_vla/test_data/vis/random_image{total_count_sub}.png')

            action = mw_policy.get_action(raw_state)
            episode_dict['image'] = obs_img
            episode_dict['point_cloud'] = obs_point_cloud[:,:3]
            episode_dict['robot_state'] = obs_robot_state
            episode_dict['action'] = action.astype(np.float64)
            episode_dict['description'] = description
            episode_arr.append(episode_dict)
            ...
            if truncated or ep_success_times>20:
                break

        if not ep_success or ep_success_times < 20:
            cprint(
            ...
            save_npy_file(
                np.array(episode_arr),
                npy_dir / f"episode_{episode_idx}.npy",
                quiet=args.quiet,
            )
