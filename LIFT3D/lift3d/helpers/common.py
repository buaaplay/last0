...
def save_rgb_image_rlbench(image_array, save_path, quiet=False):
    """save image to file
...
def save_rgb_image(image_array, save_path, quiet=False):
    ...
    Args:
        image_array (ndarray): shape (T, H, W, 3)
        save_path (str): path to save image
    """
    ...
    for i in range(len(image_array)):
        ...
        save_path = os.path.abspath(save_path)
        image.save(save_path.replace("_rgb.png", f"_rgb{i}.png"))
    if not quiet:
        Logger.log_info(f"RGB image saved to {save_path}")

def save_npy_file(npy_file, save_path, quiet=False):
    """save npy to file
    ...
    """
    save_path = os.path.abspath(save_path)
    np.save(save_path, npy_file)
    if not quiet:
        Logger.log_info(f"npy file saved to {save_path}")
